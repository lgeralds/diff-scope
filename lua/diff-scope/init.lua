print('Starting diff-scope init')
...
M.diff_dir = function(is_rec, ...)
  plat = plat or require('diff-scope.plat')
  float_buf = float_buf or require('diff-scope.float_buf')

  local ret = plat.parse_arg(...)
  print('DD: ' .. vim.inspect(ret))
  if not ret.ret then
    print("dir err")
    return
  end
...
M.close = function()
  diff_win = diff_win or require('diff-scope.diff_win')
