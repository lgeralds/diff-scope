local api = vim.api
local float_win = require('diff-scope.float_win')
local dir_diff = require('diff-scope.diff')
local diff_win = require('diff-scope.diff_win')
local plat = require('diff-scope.plat')
...
function M:init_float_buf()
  if self.float_buf_id == 0 then
    ...
    api.nvim_buf_set_keymap(
      ...
      ":lua require('diff-scope').diff_cur()<cr>",
      { silent = true }
    )
    api.nvim_buf_set_keymap(
      self.float_buf_id, 'n', '<esc>',
      ":lua require('diff-scope').close_win()<cr>",
      { silent = true }
    )
    api.nvim_buf_set_keymap(
...
function M:set_float_buf()
  ...
  self:buf_set_hls(1, #diff.change + 1, 'DiffScopeChange')
  self:buf_set_hls(#diff.change + 1, #diff.change + #diff.add + 1, 'DiffScopeAdd')
  self:buf_set_hls(#diff.change + #diff.add + 1, #buf_lines, 'DiffScopeRemove')
end
